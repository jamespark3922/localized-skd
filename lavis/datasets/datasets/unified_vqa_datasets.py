...
class UnifiedVQAGenDataset(BaseDataset):
    """ UnifiedVQA Training Dataset with Generation Objective
    """
    ...
    def __getitem__(self, index):
        ...
        question = tokens2str(tag_fn(question, references, use_descriptions))
        ...
        if is_sherlock:  # if it's sherlock it's more natural to use inference instead as answer.
            if use_descriptions: 
                region_description = answer.split(':')[-1].split('.')[0]
                question = random.choice(sherlock_inference_questions).format(region_description)
                answer = inference.split(':')[-1]
                inference = None
            elif random.random() < 0.25:  # just directly ask inference
                question = random.choice(sherlock_inference_questions).format('[0]')
                answer = inference.split(':')[-1]
                inference = None
            else:
...
class UnifiedVQAGenEvalDataset(BaseDataset):
