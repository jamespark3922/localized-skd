...
class SherlockDataset(ArrowDataset):
...
class SherlockEvalDataset(ArrowEvalDataset):
    def __init__(self, vis_processor, text_processor, arrow_root, ann_root, image_key, info, split):
        """
        arrow_root (string): directory containing *.arrow files
        ...
        with open(ann_root / 'val_comparison/val_comparison_answer_key.json') as f:
    ...
    def __getitem__(self, index):
        ...
        scores = [t['annot2'] for t in candidates] # official evaluation script uses annot2

        return {"image": image, "text_input": captions, "image_id": index, 'scores': scores}
    
    def collater(self, samples):
        image_list, caption_list, score_list, image_ids = [], [], [], []
    ...
    #     text_input = samples["text_input"] # C x B
    #     scores = samples["scores"] #
    #     batch_text = []
    #     batch_scores = []
    #     for i in range(bs):
    #         batch_text.append([t[i] for t in text_input])
    #         batch_scores.append([s[i] for s in scores])
    #     samples["text_input"] = text_input # B x C
