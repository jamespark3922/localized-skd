...
class UnifiedVQAMCDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        text_input = datum['choices']        
        text_input = [self.text_processor(tokens2str(tag_fn(text, references))) for text in text_input]
                    
        data = {"image": image, "text_input": text_input, "image_id": index}
        
        # output
        if "label" in datum:  # mc label
            data.update({"label": datum["label"], "gt_text": text_input[datum["label"]]})

        return data
    
    def collater(self, samples):
        image_list, text_list, gt_text_list, label_list, image_id_list = [], [], [], [], []
        ...
        for sample in samples:
            ...
            if "gt_text" in sample:
                gt_text_list.append(sample["gt_text"])
        ...
        if len(gt_text_list) > 0:
            data.update({"gt_text": gt_text_list})

        return data
            

class UnifiedVQAMCGenerativeDataset(BaseDataset):
