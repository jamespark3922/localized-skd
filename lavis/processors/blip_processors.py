...
@registry.register_processor("blip_unified_vqa")
class BlipQuestionAnswerProcessor(BaseProcessor):
    def __init__(self, prompt="", max_words=50):
        self.prompt = prompt
        self.max_words = max_words

    def __call__(self, question, add_prompt=True):
        if add_prompt:
            return self.prompt + self.pre_question_answer(question)
        else:
            return self.pre_question_answer(question)
    ...
    def pre_question_answer(self, question):
        question = re.sub(
            r"(.!\"()*#;~)",
            "",
            question.lower(),
